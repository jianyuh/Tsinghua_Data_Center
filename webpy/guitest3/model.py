...
def newimage_post(imagename, imagelocation, kernel, initrd):
    db.insert('imageinfo',imagename=imagename, imagelocation=imagelocation, kernel=kernel, initrd=initrd)

def delimage_post(id):
    db.delete('imageinfo',where='id=$id', vars=locals())

def updateimage_post(id, imagename, imagelocation, kernel, initrd):
    db.update('imageinfo',where='id=$id', vars=locals(), imagename=imagename, imagelocation=imagelocation, kernel=kernel, initrd=initrd)
...
#def getimagekernel(imagename):
#    try:
#        return db.select("imageinfo",what='imagelocation',where='imagename=$imagename', vars=locals())[0]
#    except IndexError:
#        return None
def getimage_post_byname(imagename):
    try:
        return db.select('imageinfo', where='imagename=$imagename', vars=locals())[0]
    except IndexError:
        return None
